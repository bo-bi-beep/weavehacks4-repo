...
`attack-kb:config` validates configuration without making a model call. `attack-kb:smoke` makes one traced OpenAI call through the recommendation-builder runtime and will consume OpenAI API usage.
...
Future issues will add schemas, recommendation logic, Redis Iris storage, curation UI, evals, seeds, and the main-agent usage skill.
